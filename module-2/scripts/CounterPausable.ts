...
async function main() {
  const contractName: any = "CounterPausable";
  ...
  const isPausedBefore = await counterPausableContract.read.isPaused();
  console.log(`\n❓ isPaused: ${isPausedBefore}\n`);
  ...
  const isPausedAfter = await counterPausableContract.read.isPaused();
